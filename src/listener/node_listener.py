...
from listener.node_transport import NodeTransport
...
class NodeListener(EventEmitter):
    ...
    async def push_advert(self, public_key, timestamp, signature, app_data):
        advert = Advert(public_key, timestamp, signature, app_data)
        await self.transport.send(advert.to_bytes())
...
# section 3
    ...
    async def handle_send_txt_msg(self, reader: BufferReader):
        """Store and echo back a text message."""
    ...
    async def handle_send_channel_txt_msg(self, reader: BufferReader):
        """Store and echo back a channel message."""
        txt_type = reader.read_uint8()
 ...
 # -------------------------
    ...
    async def handle_get_contacts(self, reader: BufferReader):
        """Return all contacts from ContactStore."""
    ...
    async def handle_add_update_contact(self, reader: BufferReader):
        """Add or update a contact in ContactStore."""
    ...
    async def handle_send_txt_msg(self, reader: BufferReader):
        """Store incoming message and echo back."""
        txt_type = reader.read_uint8()
        sender_timestamp = reader.read_uint32_le()
        pubkey_prefix = reader.read_bytes(6)
        text = reader.read_string()
    ...
    async def handle_sync_next_message(self, reader: BufferReader):
        """Return next waiting message from MessageStore."""
...
# section 4
    ...
    async def handle_remove_contact(self, reader: BufferReader):
        """Remove a contact from ContactStore."""
    ...
    async def handle_share_contact(self, reader: BufferReader):
        """Share a contact from ContactStore."""
        pubkey = reader.read_bytes(32)
        ...
        if contact:
            ...
            await self.send_err_response(Constants.ErrorCodes.NotFound)


    async def handle_export_contact(self, reader: BufferReader):
        """Export a contact from ContactStore."""
        pubkey = reader.read_bytes(32)
        contact = self.contact_store.get_contact(pubkey)
        writer = BufferWriter()
        writer.write_uint8(Constants.ResponseCodes.ExportContact)
    ...
    async def handle_import_contact(self, reader: BufferReader):
        """Import a contact into ContactStore."""
        advert_packet_bytes = reader.read_remaining_bytes()
        # Simplified: treat advert_packet_bytes as public_key
        contact = {"public_key": advert_packet_bytes, "name": "Imported"}
        self.contact_store.add_contact(contact)
        await self.send_ok_response()
...
# section 5
    ...
    async def push_msg_waiting(self):
        """Push MsgWaiting only if messages exist."""
